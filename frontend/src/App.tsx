...
export default function App() {
  ...
  const title = spec?.info.title ?? 'MFAPI — India Mutual Fund Data API'
  ...
  return (
    <div className="min-h-screen">
        ...
        {error && (
          <div className="rounded-lg border border-red-200 bg-red-50 p-4 text-sm text-red-700">
            ...
            <p className="mt-2 text-red-600">
              Is the API running? In dev, start it with <code className="font-mono">npm run dev</code> in
              the <code className="font-mono">api/</code> folder.
            </p>
          </div>
        )}
      ...
      <footer className="border-t border-slate-200 py-6 text-center text-xs text-slate-400">
        MFAPI · data from mfapi.in · built with React + Vite
      </footer>
    </div>
  )
}
